 ...
 ***********************************************************************
  ...
  Made into I2C state checking instead of while loops, PSA, KWM 2020
 */
...
// returns 0, bytesreceived, or I2CBUSY
// returns 0 even when a Master Err occurs, so need to check i2c error
/*
...
uint8_t TwoWire::getRxStatus(){
  ...
  switch(rxProcessState &0xFE){
    ...
    case RX_WAIT_BUS_BUSY: 
      ...
      if((rxBytesRemaining > 1) || !sendStop) cmd = RUN_BIT | START_BIT | ACK_BIT;
    ...
    case RX_WAIT_AFTER_DATA:
      while(1) {
        ...
        rx_error = ROM_I2CMasterErr(MASTER_BASE);
        if (rx_error != I2C_MASTER_ERR_NONE) {
          ROM_I2CMasterControl(MASTER_BASE, I2C_MASTER_CMD_BURST_RECEIVE_ERROR_STOP);
...
// new Patrick function for checking the bytes received separate from the RXStatus fn
uint8_t TwoWire::getRxBytesReceived(){
...
uint8_t TwoWire::requestFrom(uint8_t address, uint8_t quantity, uint8_t sendStop) {
  ...
  while ((error = getRxStatus()) == I2C_BUSY);
  return error;
}
  /*
uint8_t TwoWire::getRxData(unsigned long cmd) {
  ...
  switch(txProcessState & 0x7F) {
    ...
    case TX_WAIT_AFTER_DATA:
      ...
      do {
        ...
        tx_error = ROM_I2CMasterErr(MASTER_BASE);
        // bail on error.
        if (tx_error != I2C_MASTER_ERR_NONE) {
          ROM_I2CMasterControl(MASTER_BASE, I2C_MASTER_CMD_BURST_SEND_ERROR_STOP);
...
{
	txWriteIndex = 0;
  txReadIndex=0;  // keith addition because obviously things can still read 
  //the next readindex and have that be written to the device even though flush 
  //implies you wouldnt want that next byte whatever it is to go but instead what will be put next into the buffer. 
	rxReadIndex = rxWriteIndex; // keith added because he needs this to be zero if we are going to fall through, unitialized seems to provide weird behavior. 
